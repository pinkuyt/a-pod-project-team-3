...
unsigned char Max(unsigned int servo)
{
	if (Servos[servo] <2100) return 0;
	return 1;
}
unsigned char Min(unsigned int servo)
{
	if (Servos[servo] >900) return 0;
	return 1;
}
// ----------------------------------------
// Mandible control
...
{
	int interval =  2000 - Servos[Mandible[0]];
	Servos[Mandible[0]] = 2000;
	Servos[Mandible[1]] = 1000;
	return interval;
}
...
// normal moves
void APOD_Forward(int loop, int intervalVertical, int intervalHorizontal)
{
	char cmd[74];
	char i = 0;
	int step = intervalVertical/20;
	uint32_t delay = 10*step*APOD_Step_interval;
	// --------------------------------
...
void APOD_Backward(int loop, int intervalVertical, int intervalHorizontal)
{
	char cmd[74];
	char i = 0;
	int step = intervalVertical/20;
...
void APOD_TurnLeft(int loop, int intervalVertical, int intervalHorizontal)
{
	char cmd[74];
	char i = 0;
	int step = intervalVertical/20;
...
void APOD_TurnRight(int loop, int intervalVertical, int intervalHorizontal)
{
	char cmd[74];
	char i = 0;
	int step = intervalVertical/20;
...
// Advance moves
void APOD_Forward_Advance(int intervalVertical, int intervalHorizontal)
{
	char cmd[74];
	int step = intervalVertical/20;
	uint32_t delay = 10*step*APOD_Step_interval;
...
void APOD_Backward_Advance(int intervalVertical, int intervalHorizontal)
{
	char cmd[74];
	int step = intervalVertical/20;
	uint32_t delay = 10*step*APOD_Step_interval;
...
void APOD_TurnLeft_Advance(int intervalVertical, int intervalHorizontal)
{
...
void APOD_TurnRight_Advance(int intervalVertical, int intervalHorizontal)
{
	char cmd[74];
	int step = intervalVertical/20;
	uint32_t delay = 10*step*APOD_Step_interval;
...
void Apod_Squeeze_Right(int interval)
{
	...
	if (Min(RightFront[1])) return;
	
	LEG_Lift(LEFT_FRONT,interval);
	LEG_Lift(LEFT_CENTER,interval);
	LEG_Lift(LEFT_REAR,interval);
	
	LEG_Drop(RIGHT_FRONT,interval);
	LEG_Drop(RIGHT_CENTER,interval);
	LEG_Drop(RIGHT_REAR,interval);
...
unsigned int Apod_Read_Distance(void)
{
	...
	for(i=0;i<8;i++)
	{
		...
		HighByte = USART2_GetData();
		LowByte = USART2_GetData();
		
		value += (((HighByte<<8)+LowByte)/58);
	}
	value >>=3;
	return value;
}
